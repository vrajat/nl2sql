...
RUN python -m venv /opt/nl2sql-env
ENV PATH /opt/nl2sql-env/bin:$PATH
ENV VIRTUAL_ENV /opt/nl2sql-env

# Set the working directory.
WORKDIR /workspaces/nl2sql/

# Install the run time Python dependencies in the virtual environment.
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/nl2sql/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/nl2sql/ && touch src/nl2sql/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/nl2sql/
...
ENTRYPOINT ["/opt/nl2sql-env/bin/nl2sql"]
CMD []
